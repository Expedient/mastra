...
description: "Implement IFGAProvider to authorize users and system actors against Mastra resources with fine-grained authorization checks and provider-specific policies."
packages:
  - "@mastra/core"
---
...
For concepts, configuration, and the lifecycle points where Mastra enforces FGA, see [Fine-grained authorization](/docs/auth/fga).
...
import { FGADeniedError } from '@mastra/core/auth/authorization'
import type {
  FGACheckParams,
  IFGAProvider,
...
Throws `FGADeniedError` when `user` lacks the permission. Mastra calls this at its [enforcement points](/docs/auth/fga#enforcement-points).
...
Treat `actor.permissions` as an untrusted claim. Resolve the agent's authoritative grants from a trusted source keyed by `actor.agentId`, rather than trusting the inline values. See [System actors](/docs/auth/fga#system-actors).

```typescript
import { FGADeniedError } from '@mastra/core/auth/authorization'
import type { ActorSignal, FGACheckParams, IFGAProvider } from '@mastra/core/auth/authorization'
...
<PropertiesTable
  ...
  },
  {
    ...
    defaultValue: 'false',
    description:
      "Forwards this actor into the agent and tool calls the framework makes for the run's declarative .then(agent) and .then(tool) steps. Custom step execute functions aren't covered and must pass actor explicitly. Ignored by authorization itself.",
  },
]}
/>
...
- [Fine-grained authorization](/docs/auth/fga)
- [System actors](/docs/auth/fga#system-actors)
